...
export async function checkUsernameAvailable(
  ...
  const { data, error } = await getClient().rpc("is_username_available", {
    p_username: username,
  });
...
export async function sendSecurityCode(): Promise<SendSecurityCodeResult> {
  const { data, error } =
    await getClient().functions.invoke("send-security-code");
  if (error) {
    return {
      ok: false,
...
async function sendAccountAlreadyExistsEmail(email: string): Promise<void> {
  try {
    await getClient().functions.invoke("send-account-exists-email", {
      body: { email },
    });
  } catch {
