...
export type FormCreateFormData = {
  name: string;
  description?: string;
};
...
interface FormCreateModalProps {
  onClose: () => void;
  onCreate: (data: FormCreateFormData) => Promise<void>;
}
  ...
  return (
    <div className="flex flex-col gap-4 px-6 py-5">
      <PortalInput
        ...
        placeholder="e.g. Pre-Workout Check-In"
        autoFocus
      />
      <PortalTextarea
      ...
      <p className="text-portal-text3 text-xs">
        You&apos;ll add questions after creating the form.
      </p>
    </div>
  );
}
...
export function FormCreateModal({ onClose, onCreate }: FormCreateModalProps) {
