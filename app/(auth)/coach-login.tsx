...
export default function CoachLoginScreen() {
  ...
  async function handleSignIn() {
    ...
    const result = await signInWithUsername(username.trim(), password, true);
    if (!result.ok) {
    ...
    <StyledSafeAreaView className="bg-surface flex-1" edges={["top"]}>
      <View className="flex-1">
        ...
        <View className="px-6 pt-2 pb-4">
          ...
          <Hero className="mb-1">Coach sign in</Hero>
          <BodySm>Sign in to your coaching account</BodySm>
        </View>
        ...
        <KeyboardStickyView>
          <StyledSafeAreaView edges={["bottom"]} className="bg-surface">
            <View className="gap-3 px-6 py-3">
              ...
              <Caption className="text-center text-text-tertiary">
                Coach accounts only
              </Caption>
            </View>
          </StyledSafeAreaView>
